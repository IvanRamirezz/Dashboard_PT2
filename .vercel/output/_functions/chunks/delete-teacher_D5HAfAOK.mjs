...
async function POST({ request, cookies }) {
  ...
  try {
    const { data: usuario, error: userError } = await supabaseAdmin.from("usuarios").select("auth_uid").eq("usuario_id", profesor_id).single();
    ...
    const { error: dbError } = await supabaseAdmin.from("usuarios").delete().eq("usuario_id", profesor_id);
    if (dbError) {
